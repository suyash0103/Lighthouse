...
void rotateBeam()
{
    ...
    printf("%R is : %d\n", r);
    printf("BEAM Y isss : %f\n", beam_y1);
    if(beam_y1 > 0 && r % 2 == 0)
    {
        beam_x1 = ((float)(beam_x1)*cos(theta)) + ((float)(beam_y1)*sin(theta));
        beam_y1 = ((float)(beam_x1)*cos(theta)) - ((float)(beam_y1)*sin(theta));
        printf("BEAM Y is : %f\n", beam_y1);
        r = 2;
        printf("1\n");
    }

    else if(beam_y1 < 100 * scale_y)
        r = 1;

    if(beam_y1 < 400 * scale_y && r % 2 != 0)
    {
        beam_x1 = ((float)(beam_x1)*cos(theta)) - ((float)(beam_y1)*sin(theta));
        beam_y1 = ((float)(beam_x1)*cos(theta)) + ((float)(beam_y1)*sin(theta));
        r = 1;
